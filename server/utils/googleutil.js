const {google} = require('googleapis');
const config = require('../config/config.json');

const Util = require('../utils/globalutility.js');
const utilInstance = new Util();

/**
 * This scope tells google what information we want to request.
 */
const defaultScope = [
    'https://www.googleapis.com/auth/userinfo.email',
    'https://www.googleapis.com/auth/userinfo.profile'
];

class GoogleUtil{
    constructor(){}

    /**
     * Create the google auth object which gives us access to talk to google's apis.
     */
    async createConnection(){
        return new google.auth.OAuth2(
            config.login.oAuth2.google.clientId,
            config.login.oAuth2.google.clientSecret,
            config.login.oAuth2.google.redirect
        );
    }   
    

    /**
     * Get a url which will open the google sign-in page and request access to the scope provided (such as calendar events).
     */
    async getConnectionUrl(auth) {
        return auth.generateAuthUrl({
          access_type: 'offline',
          prompt: 'consent', // access type and approval prompt will force a new refresh token to be made each time signs in
          scope: defaultScope
        });
      }

    /**
     * Create the google url to be sent to the client.
     */
    async urlGoogle() {
        var auth = await this.createConnection(); // this is from previous step
        const url = await this.getConnectionUrl(auth);
        return url;
    }

    /**
    * Helper function to get the library with access to the google plus api.
    */
    async getGooglePlusApi(auth) {
        return google.plus({ version: 'v1', auth });
    }

    /**
     * Extract the email and id of the google account from the "code" parameter.
     */
    async getGoogleAccountFromCode(code) {
        
        const auth = await this.createConnection();

        // get the auth "tokens" from the request
        const data = await auth.getToken(code);
        const tokens = data.tokens;
        
        // add the tokens to the google api so we have access to the account
        
        auth.setCredentials(tokens);
        
        // connect to google plus - need this to get the user's email
        const plus = await this.getGooglePlusApi(auth);
        const me = await plus.people.get({ userId: 'me' });
        
        // get the google id and email
        const userGoogleId = me.data.id;
        const userGoogleEmail = me.data.emails && me.data.emails.length && me.data.emails[0].value;

        // return so we can login or sign up the user
        return {
            id: userGoogleId,
            email: userGoogleEmail,
            tokens: tokens, // you can save these to the user if you ever want to get their details without making them log in again
        };
    }

    async getToken(code){
         // get the auth "tokens" from the request
         var auth = await this.createConnection();
         var data = await auth.getToken(code);
         var tokens = data.tokens;
         return tokens;
    }

}

module.exports = GoogleUtil;